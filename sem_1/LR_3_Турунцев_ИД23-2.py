...
class Dialogue(QDialog):
    ...
    # Добавляем поля ввода position, size, droplet_density, droplet_speed
        ...
        self.bias = QLabel("Введите смещение капель")
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        Pause_Animation_btn = QPushButton("Pause", self)
        Pause_Animation_btn.move(200, 0)
        Pause_Animation_btn.setStyleSheet("background-color: black; color: white;")

        Add_Cloud_btn.clicked.connect(self.Add_Cloud_btn_clicked)
        Delete_Cloud_btn.clicked.connect(self.Delete_Cloud_btn_clicked)
        Pause_Animation_btn.clicked.connect(self.Pause_Animation_btn_clicked)
    ...
    def Add_Cloud_btn_clicked(self):
        ...
        class Cloud():
            ...
            def updateDrops(self, bias = random.randint(-3, 3), speed = random.randint(20, 50)):
                ...
                for drop in self.drops_fixed:
                        ...
                        drop[0] += self.droplet_bias
                
                if random.random() < random.randint(0, 1):  # С вероятностью добавляем новую каплю
                    x = random.randint(self.position[0], self.position[0] + self.size - 10)
                    self.drops_random.append([x, self.position[1] + self.size, 2, self.size // random.randint(2, 4)])
                for drop in self.drops_random:
                        drop[1] += speed  # Двигаем капли вниз
                        drop[0] += bias                

                self.drops_fixed = [drop for drop in self.drops_fixed if drop[1] < 800]
                self.drops_random = [drop for drop in self.drops_random if drop[1] < 800]  # Удаляем капли за пределами экрана   
                self.upd()        
            ...
            def open_dialog(self, event):
                ...
                if dialog.exec_() == QDialog.Accepted:
                    ...
                    self.droplet_bias = int(dialog.bias_input.text())
            


        with open(r'C:/Users/petrt/Desktop/VS_Projects/LR_3/cloud_config.json', 'r', encoding='utf-8') as json_file:
            data = json.load(json_file)


        new_cloud = Cloud(data['position'], data['size'], data['droplet_density'], data['droplet_speed'], self.update)
    ...
    def Delete_Cloud_btn_clicked(self):
        self.Clouds.remove(self.current_cloud)
        self.update()
    
    def Pause_Animation_btn_clicked(self):
        if self.paused:
